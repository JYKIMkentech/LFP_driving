...
% Add C-rate and scaled current to the UDDS_unit table
UDDS_unit.C_rate = C_rate;
UDDS_unit.scaled_current = scaled_current;

% Create a structure to store results
results = struct();
results.total_distance_km = sum(speed_ms(1:end-1) .* dt) / 1000;
